const
	...
	nameRegistryAddress = '0xffe08afb7fe148c42deba7ff80d6b9444ce95b6a'
...
async function loopKeys(contract, did) {
	...
	for (let i = keysCount.length - 1; i >= 0; i--) {
		const
			hash = await contract.methods.keyHashes(i).call()
			gotKey = await contract.methods.getKeyByHash(hash).call()
			key = {
				id: `${did}#keys-` + (i + 1),
...
async function loopServices(contract) {
	...
	for (let i = servicesCount.length - 1; i >= 0; i--) {
		const
			serviceType = await contract.methods.services(i).call()
			endpoint = await contract.methods.getServiceByType(serviceType).call()
			service = {
...
async function buildDocument(did, address) {
	...
	if (await web3.eth.getCode(address) != "0x") {
		const
			contract = await identityContractConnector(address)
